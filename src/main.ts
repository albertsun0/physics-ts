  ...
  public update() {
    for (let i = 0; i < this.subSteps; i++) {
      ...
      for (let i = 0; i < this.objects.length; i++) {
        this.applyAngleConstraint(this.objects[i]);
      }
  ...
  public applyConstraintCircle() {
    ...
    for (let i = 0; i < this.objects.length; i++) {
      ...
      if (dist > radius - this.objects[i].radius) {
        const n = diff.mult(1 / dist);
        this.objects[i].position_current = position.add(
          n.mult(radius - this.objects[i].radius)
        );
      }
    }
  }

  public applyAngleConstraint(o: VerletObject) {
    // find all links between object
    const links = this.links.filter((l) => l.a === o || l.b === o);
    if (links.length !== 2) return;

    const a = links[0];
    const b = links[1];

    const ao = a.a.id !== o.id ? a.a : a.b;
    const bo = b.a.id !== o.id ? b.a : b.b;

    const va = ao.position_current.sub(o.position_current);
    const vb = bo.position_current.sub(o.position_current);

    const thetaA =
      Math.acos(va.dot(vb) / (Vec2Length(va) * Vec2Length(vb))) - Math.PI;

    const thetaB =
      Math.acos(vb.dot(va) / (Vec2Length(va) * Vec2Length(vb))) - Math.PI;

    // console.log("theta", (theta + Math.PI) * (180 / Math.PI));
    const va_perp = Vec2Rotate90(va).mult(1 / Vec2Length(va));
    const vb_perp = Vec2Rotate90(vb).mult(1 / Vec2Length(vb));

    ao.position_current = ao.position_current.add(
      va_perp.mult(-0.01 * thetaA).mult(thetaA)
    );
    bo.position_current = bo.position_current.add(
      vb_perp.mult(-0.01 * thetaB).mult(thetaB)
    );
  }
  ...
  public applyConstraint() {
    for (let i = 0; i < this.objects.length; i++) {
      ...
      if (object.position_current.y > this.height - object.radius) {
        ...
        object.position_current = object.position_current.add(
          u.mult(diff * bounceFactor)
        );
      }
    ...
    for (let i = 0; i < this.objects.length; i++) {
      ...
      for (let j = i + 1; j < this.objects.length; j++) {
        ...
        if (dist < minDist) {
          ...
          obj1.position_current = obj1.position_current.add(
            n.mult(0.5 * delta).mult(0.99)
          );
          obj2.position_current = obj2.position_current.sub(
            n.mult(0.5 * delta).mult(0.99)
          );
        }
      }
